...
(function () {
    ...
    // ML-288: no inline styles - examples use the same classes as the app. The only style="" left is a
    // custom property carrying a run-time value (a position, a bar height, the token a table row shows),
    // exactly as the app sets it from JS.

    const dot = (cls, left, extra = '') => `<span class="metro-dot ${cls}" style="--dot-x:${left}%">${extra}</span>`;
    ...
    const layoutIcon1 = '<svg class="flow-layout-icon" viewBox="0 0 100 100" aria-hidden="true" fill="currentColor"><rect x="0" y="0" width="100" height="42" rx="3"/><rect x="0" y="58" width="100" height="42" rx="3"/></svg>';
    const layoutIcon2 = '<svg class="flow-layout-icon" viewBox="0 0 100 100" aria-hidden="true" fill="currentColor"><rect x="0" y="0" width="42" height="42" rx="3"/><rect x="58" y="0" width="42" height="42" rx="3"/><rect x="0" y="58" width="42" height="42" rx="3"/><rect x="58" y="58" width="42" height="42" rx="3"/></svg>';
    ...
    const nRow = (svgs, k) => `<div class="flex-row gap-md flex-wrap items-end">${svgs.map(s => nScale(s, k)).join('')}</div>`;
    ...
    const notationExamples = NT ? [
        ...
        { label: 'Metronome marks (ML-297): a Bravura crotchet, then "= 108" in the tempo-word style (Notation.tempoMark) - the Speeds quiz asks which speed each is', wide: true, html: nRow([NT.tempoMark(52), NT.tempoMark(108), NT.tempoMark(184)]), measure: '.notation' },
    ...
    const theoryAnswer = (label, state, id) => `<button type="button" class="theory-answer${state ? ` theory-answer-${state}` : ''}"${id ? ` data-id="${id}"` : ''}>${state ? `<span class="material-symbols-outlined" aria-hidden="true">${state === 'right' ? 'check' : 'close'}</span>` : ''}${label}</button>`;
    ...
    const GROUPS = [
        {
            ...
            items: [
                ...
                { spec: 'tool-icon-button', title: 'Tool icon buttons', examples: [
                    { label: 'Tool groups: a small label (.tool-group-title) over each group row - Everyday, Practise, Learn', html: '<div><div class="tool-group"><div class="tool-group-title">Everyday</div><div class="tool-icon-row"><button class="tool-icon-btn" type="button"><span class="material-symbols-outlined">timer</span><span class="tool-icon-label">Timer</span></button><button class="tool-icon-btn" type="button"><span class="material-symbols-outlined">graphic_eq</span><span class="tool-icon-label">Tuner</span></button></div></div><div class="tool-group"><div class="tool-group-title">Learn</div><div class="tool-icon-row"><button class="tool-icon-btn" type="button"><span class="material-symbols-outlined">touch_app</span><span class="tool-icon-label">Tempo</span></button><button class="tool-icon-btn" type="button"><span class="material-symbols-outlined">graphic_eq</span><span class="tool-icon-label">Pulse</span></button><button class="tool-icon-btn" type="button"><span class="material-symbols-outlined">hearing</span><span class="tool-icon-label">Pitch</span></button></div></div></div>', measure: '.tool-group-title, .tool-icon-row' },
        ...
        },
        {
            ...
            items: [
                ...
                { spec: 'dropdown-menu', title: 'Dropdown menu', examples: [
                    ...
                    }<div class="nav-divider" role="separator"></div><div class="nav-section-title">Tools</div><div class="nav-tools" role="group" aria-label="Tools">${[['avg_pace', 'Metronome'], ['grid_view', 'Flow'], ['graphic_eq', 'Tuner'], ['timer', 'Timer'], ['music_note', 'Theory']].map(([i, t]) => `<button type="button" class="nav-tool"><span class="material-symbols-outlined" aria-hidden="true">${i}</span><span class="nav-tool-label">${t}</span></button>`).join('')}</div><div class="nav-divider" role="separator"></div><div class="nav-section-title">Progress</div><button type="button" class="dropdown-item nav-item"><span class="material-symbols-outlined nav-item-icon" aria-hidden="true">bar_chart</span><span class="nav-item-text">Stats</span></button><button type="button" class="dropdown-item nav-item" aria-current="page"><span class="material-symbols-outlined nav-item-icon" aria-hidden="true">local_fire_department</span><span class="nav-item-text">Streaks</span></button><div class="nav-divider" role="separator"></div><div class="nav-section-title">You</div><button type="button" class="dropdown-item nav-item"><span class="material-symbols-outlined nav-item-icon" aria-hidden="true">person</span><span class="nav-item-text">My account</span></button><div class="nav-divider" role="separator"></div><button type="button" class="dropdown-item nav-item"><span class="material-symbols-outlined nav-item-icon" aria-hidden="true">info</span><span class="nav-item-text">About</span><span class="nav-item-meta">v0.27.0</span></button><div class="nav-divider" role="separator"></div><button type="button" class="dropdown-item nav-item"><span class="material-symbols-outlined nav-item-icon" aria-hidden="true">logout</span><span class="nav-item-text">Log out<span class="nav-item-sub">you@example.com</span></span></button></div></div>`, measure: '.nav-item, .nav-item[aria-current], .nav-section-title, .nav-tool, .nav-item-meta, .nav-item-sub, .nav-divider' },
                    { label: '⋮ menus with icons (ML-285): Play Flow and a Flow library row - Material Symbols for standard actions, custom SVGs (public/icons/menu/) for app-specific ones, a delete item red', html: '<div class="admin-design-static flex-row gap-md"><div class="dropdown-menu show"><button type="button" class="dropdown-item"><span class="material-symbols-outlined dropdown-item-icon" aria-hidden="true">edit_note</span><span class="dropdown-item-text">Edit details</span></button><button type="button" class="dropdown-item"><svg class="dropdown-item-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M11 4H6a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-5"/><path d="M11 13.5v-2.7l6.6-6.6a1.5 1.5 0 0 1 2.1 0l1.1 1.1a1.5 1.5 0 0 1 0 2.1L14.2 13.5z"/><circle cx="7.5" cy="16.5" r="1" fill="currentColor" stroke="none"/><circle cx="11" cy="16.5" r="1" fill="currentColor" stroke="none"/><circle cx="14.5" cy="16.5" r="1" fill="currentColor" stroke="none"/><circle cx="7.5" cy="13" r="1" fill="currentColor" stroke="none"/></svg><span class="dropdown-item-text">Edit bars</span></button><button type="button" class="dropdown-item"><svg class="dropdown-item-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M7 3h12a2 2 0 0 1 2 2v12"/><path d="M5 7h10a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V9a2 2 0 0 1 2-2z"/><path d="M10 10.5v6.5"/><path d="M7 14.5l3 3 3-3"/></svg><span class="dropdown-item-text">Load from library</span></button><button type="button" class="dropdown-item"><span class="material-symbols-outlined dropdown-item-icon" aria-hidden="true">add</span><span class="dropdown-item-text">Create new</span></button></div><div class="dropdown-menu show"><button type="button" class="dropdown-item"><span class="material-symbols-outlined dropdown-item-icon" aria-hidden="true">edit</span><span class="dropdown-item-text">Edit</span></button><button type="button" class="dropdown-item"><span class="material-symbols-outlined dropdown-item-icon" aria-hidden="true">content_copy</span><span class="dropdown-item-text">Duplicate</span></button><!-- ML-204: personal + band flows, gated by flow_export_musicxml (openFlowLibraryItemMenu) --><button type="button" class="dropdown-item"><svg class="dropdown-item-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M14 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V8z"/><path d="M14 3v5h5"/><path d="M12 18v-7"/><path d="M9 13.5l3-3 3 3"/></svg><span class="dropdown-item-text">Export to MusicXML</span></button><button type="button" class="dropdown-item metroBlk-tile-menu-delete"><span class="material-symbols-outlined dropdown-item-icon" aria-hidden="true">delete</span><span class="dropdown-item-text">Delete flow</span></button></div></div>', measure: '.dropdown-item-icon, .dropdown-item-text' },
        ...
        },
        {
            ...
            items: [
                ...
                { spec: 'list-row', title: 'List rows', examples: [
                    { label: 'Display rows (session history): page surface, only the ⋮ menu is tappable', html: '<div><div class="history-item category-edge category-practise"><div class="history-details"><strong class="category-text">Practise</strong> 3 Sep 2026 | 25 mins</div><button type="button" class="list-item-menu-btn" aria-label="Options" aria-haspopup="menu" aria-expanded="false"><span class="material-symbols-outlined">more_vert</span></button></div><div class="history-item category-edge category-lesson"><div class="history-details"><strong class="category-text">Lesson</strong> 1 Sep 2026 | 45 mins</div><button type="button" class="list-item-menu-btn" aria-label="Options" aria-haspopup="menu" aria-expanded="false"><span class="material-symbols-outlined">more_vert</span></button></div></div>', measure: '.history-item' },
                    { label: 'Tappable rows (.history-item.clickable - Flow library, saved setups, Quick-play history; or a <button> row - challenges)', html: '<div><div class="history-item clickable"><div class="history-details"><strong>Clarinet Concerto</strong><span class="text-muted">32 bars · Personal</span></div><button type="button" class="list-item-menu-btn" aria-label="Options" aria-haspopup="menu" aria-expanded="false"><span class="material-symbols-outlined">more_vert</span></button></div><button type="button" class="history-item category-edge category-practise"><div class="history-details"><strong>30-day scales challenge</strong><span class="text-muted">12 of 30 days</span></div></button></div>', measure: '.history-item.clickable, button.history-item' },
                    { label: 'Settings groups (button.history-item.settings-link, ML-282): icon, title, summary of current values, chevron; each opens its own Settings screen', html: '<div><button type="button" class="history-item settings-link"><span class="material-symbols-outlined settings-link-icon" aria-hidden="true">contrast</span><span class="settings-link-text"><span class="settings-link-title">Display</span><span class="settings-link-sub">Dark mode on</span></span><span class="material-symbols-outlined settings-link-chevron" aria-hidden="true">chevron_right</span></button><button type="button" class="history-item settings-link"><span class="material-symbols-outlined settings-link-icon" aria-hidden="true">bar_chart</span><span class="settings-link-text"><span class="settings-link-title">Stats</span><span class="settings-link-sub">Calendar year · projection shown</span></span><span class="material-symbols-outlined settings-link-chevron" aria-hidden="true">chevron_right</span></button></div>', measure: '.settings-link, .settings-link-sub' },
                ...
                { spec: 'charts', title: 'Charts', examples: [
                    ...
                    { label: 'Bar colours: hours / days / sessions / streak (all gold, ML-235) / category (only for a chart split by session type)', html: `<div class="admin-design-bars">${[['--chart-hours', 70], ['--chart-days', 45], ['--chart-sessions', 85], ['--chart-streak', 60], ['--cat-rehearsal', 30], ['--cat-performance', 55]].map(([t, h]) => `<div class="chart-bar" style="--bar-h:${h}%; --series: var(${t})"></div>`).join('')}</div>`, measure: '.chart-bar' },
                    { label: 'Projected month (ML-186), with its Actual / Projected key (.chart-legend): the current month\'s hollow outline bar (.chart-bar-projection, border in the chart\'s own colour) stacked on the real bar (.chart-bar-under-projection) - "at this rate" by month end', html: `<div class="admin-design-bars">${[70, 85, 60].map(h => `<div class="chart-bar series-hours" style="--bar-h:${h}%"></div>`).join('')}<div class="chart-bar-container admin-design-bar-slot"><div class="chart-bar-projection series-hours" style="--bar-h:30%"></div><div class="chart-bar chart-bar-under-projection series-hours" style="--bar-h:45%"></div></div></div><div class="chart-legend"><span class="chart-legend-item"><span class="chart-legend-swatch series-hours"></span>Actual</span><span class="chart-legend-item"><span class="chart-legend-swatch chart-legend-swatch-projected series-hours"></span>Projected</span></div>`, measure: '.chart-bar-projection, .chart-legend, .chart-legend-swatch-projected' },
                ] },
            ],
        },
        {
            ...
            items: [
                { spec: 'modal', title: 'Modal', examples: [
                    { label: 'Destructive confirmation (shown in place, not over the page)', html: '<div class="admin-design-static"><div class="modal" role="dialog" aria-modal="true" aria-label="Delete session? (example)"><div class="modal-content"><button class="modal-close-x" type="button" aria-label="Close">✕</button><h2>Delete session?</h2><p>This can\'t be undone.</p><div class="flex-row gap-md"><button class="btn-nav btn-cancel" type="button">Cancel</button><button class="btn-submit is-danger" type="button">Delete</button></div></div></div></div>', measure: '.modal, .modal-content, .modal-content h2' },
                ] },
        ...
        },
        {
            ...
            items: [
                { spec: 'metronome', title: 'Metronome', examples: [
                    { label: 'Tempo box with the speed name under the bpm (.metro-speed-name, ML-297) - a band with two names shows both', html: `<div class="flex-row gap-md flex-wrap"><div class="metroBlk-bpm-box"><div class="metro-speed-row no-margin"><button class="metro-bpm-step" type="button" aria-label="Decrease beats per minute">&minus;</button><div class="metro-speed-readout"><div>108</div><div class="metro-speed-sub">bpm</div><div class="metro-speed-name">Moderato</div></div><button class="metro-bpm-step" type="button" aria-label="Increase beats per minute">+</button></div></div><div class="metroBlk-bpm-box"><div class="metro-speed-row no-margin"><button class="metro-bpm-step" type="button" aria-label="Decrease beats per minute">&minus;</button><div class="metro-speed-readout"><div>66</div><div class="metro-speed-sub">bpm</div><div class="metro-speed-name">Adagio / Lento</div></div><button class="metro-bpm-step" type="button" aria-label="Increase beats per minute">+</button></div></div></div>`, measure: '.metro-speed-readout, .metro-speed-name' },
                ...
                { spec: 'warmups', title: 'Warm-ups', examples: NT && window.Warmups ? [
                    { label: 'Previous / Next / Shuffle / My warm-ups (the same row as Scales)', html: `<div class="scales-pick-row"><button type="button" class="metro-play-grid-btn metro-play-grid-btn-reset"><span class="material-symbols-outlined" aria-hidden="true">skip_previous</span><span class="metroBlk-ctrl-value-label">previous</span></button><button type="button" class="metro-play-grid-btn metro-play-grid-btn-reset"><span class="material-symbols-outlined" aria-hidden="true">skip_next</span><span class="metroBlk-ctrl-value-label">next</span></button><button type="button" class="metro-play-grid-btn metro-play-grid-btn-reset"><span class="material-symbols-outlined" aria-hidden="true">shuffle</span><span class="metroBlk-ctrl-value-label">shuffle</span></button><button type="button" class="metro-play-grid-btn scales-pick-mine"><span class="material-symbols-outlined" aria-hidden="true">playlist_add_check</span><span class="metroBlk-ctrl-value-label">my warm-ups</span></button></div>`, measure: '.metro-play-grid-btn' },
                    { label: 'The exercise card: title, kind and place, tip, the stave (a whole-bar rest centred), the 3rd note playing', wide: true, html: `<section class="flow-card scales-staff-card"><div class="scales-staff-head"><div class="scales-staff-heading"><h2 class="scales-staff-title">Long tones down from G</h2><span class="scales-staff-sub">Long tones · 1 of 10</span></div></div><p class="warmups-tip">Full breath before each note. Keep the sound the same from start to finish.</p><div class="scales-staff">${(() => { const ex = { beatsPerBar: 4, notes: window.Warmups.parse('G4w rw F#4w rw') }; return window.Warmups.rows(ex, 'treble').map(r => NT.staff({ clef: 'treble', items: r.items.map((it, i) => it.cls && /warmup-note-2/.test(it.cls) ? { ...it, cls: it.cls + ' is-now' } : it), stepRange: window.Warmups.stepRange(ex), justify: 60 })).join(''); })()}</div></section>`, measure: '.scales-staff-title, .warmups-tip, .warmup-note.is-now' },
                    { label: 'Admin editor pieces: the preview with a note selected (gold) and the status line', wide: true, html: `<div><div class="admin-warmup-preview">${(() => { const ex = { beatsPerBar: 4, notes: window.Warmups.parse('G4h C5h G4w') }; return window.Warmups.rows(ex).map(r => NT.staff({ clef: 'treble', items: r.items.map(it => it.cls && /warmup-note-1/.test(it.cls) ? { ...it, cls: it.cls + ' is-selected' } : it), justify: 60 })).join(''); })()}</div><p class="admin-warmup-status">3 notes, 2 bars. Note 2 selected: a tap on a note replaces it.</p><p class="admin-warmup-status has-errors">Note 2 runs over the bar line (bar 1).</p></div>`, measure: '.admin-warmup-preview, .admin-warmup-status' },
                ] : [] },
                { spec: 'drills', title: 'Drills (Tempo, Pulse, Pitch)', examples: [
                    { label: 'Tempo: the metronome mark, the live meter (a little slow), the feedback line and the pad', html: `<div><p class="theory-question">Tap at this speed</p><div class="theory-prompt">${NT ? nScale(NT.tempoMark(116, { label: 'Crotchet equals 116' }), 1.4) : ''}</div><div class="drill-meter" aria-hidden="true"><div class="drill-meter-track"><span class="drill-meter-mark"></span><span class="drill-meter-needle" style="--pos:-0.4"></span></div><div class="drill-meter-labels"><span>slower</span><span>on it</span><span>faster</span></div></div><p class="drill-feedback">A bit faster</p><button type="button" class="drill-pad">Tap</button></div>`, measure: '.drill-meter-track, .drill-meter-needle, .drill-feedback, .drill-pad' },
                    { label: 'The pad pressed (.is-hit) and done (disabled)', html: '<div class="flex-row gap-md"><button type="button" class="drill-pad is-hit">Tap</button><button type="button" class="drill-pad" disabled>Tap</button></div>', measure: '.drill-pad' },
                    { label: 'Pulse: 8 bars of 4 beats, 2 on 2 off - a silent beat is an empty ring (.is-silent), the beat now gold (.is-now)', wide: true, html: '<div class="drill-beats"><div class="drill-bar"><span class="drill-beat"></span><span class="drill-beat"></span><span class="drill-beat"></span><span class="drill-beat"></span></div><div class="drill-bar"><span class="drill-beat is-now"></span><span class="drill-beat"></span><span class="drill-beat"></span><span class="drill-beat"></span></div><div class="drill-bar"><span class="drill-beat is-silent"></span><span class="drill-beat is-silent"></span><span class="drill-beat is-silent"></span><span class="drill-beat is-silent"></span></div><div class="drill-bar"><span class="drill-beat is-silent"></span><span class="drill-beat is-silent"></span><span class="drill-beat is-silent"></span><span class="drill-beat is-silent"></span></div><div class="drill-bar"><span class="drill-beat"></span><span class="drill-beat"></span><span class="drill-beat"></span><span class="drill-beat"></span></div><div class="drill-bar"><span class="drill-beat"></span><span class="drill-beat"></span><span class="drill-beat"></span><span class="drill-beat"></span></div><div class="drill-bar"><span class="drill-beat is-silent"></span><span class="drill-beat is-silent"></span><span class="drill-beat is-silent"></span><span class="drill-beat is-silent"></span></div><div class="drill-bar"><span class="drill-beat is-silent"></span><span class="drill-beat is-silent"></span><span class="drill-beat is-silent"></span><span class="drill-beat is-silent"></span></div></div>', measure: '.drill-beats, .drill-bar, .drill-beat, .drill-beat.is-silent, .drill-beat.is-now' },
                    { label: 'Pitch: waiting for an answer (.drill-ear-icon), and Play it back listening (.drill-listen)', html: '<div><div class="theory-prompt"><span class="material-symbols-outlined drill-ear-icon" aria-hidden="true">hearing</span></div><p class="drill-listen">Listening… <strong>G</strong></p></div>', measure: '.drill-ear-icon, .drill-listen' },
                    { label: 'Results: a line per speed / gap / note (.drill-result-list)', html: '<ul class="drill-result-list"><li><span>116 bpm</span><strong>104 bpm · 51</strong></li><li><span>Moderato (108-119)</span><strong>112 bpm · 100</strong></li><li><span>Back in after gap 1</span><strong>32 ms late</strong></li></ul>', measure: '.drill-result-list li' },
                ] },
                { spec: 'scales', title: 'Scales practice', examples: NT ? [
                    { label: 'Setup tiles, Previous / Next / Shuffle / My scales, and the stave: D major, one octave up, key signature, the 3rd note playing (gold)', wide: true, html: `<div><div class="flow-tile-grid flow-tile-grid-4"><button type="button" class="flow-tile"><span class="flow-tile-value">D</span><span class="flow-tile-label">major</span></button><button type="button" class="flow-tile"><span class="flow-tile-value">Scale</span><span class="flow-tile-label">type</span></button><button type="button" class="flow-tile"><span class="flow-tile-value">1</span><span class="flow-tile-label">octave</span></button><button type="button" class="flow-tile"><span class="flow-tile-value"><span class="material-symbols-outlined" aria-hidden="true">arrow_upward</span></span><span class="flow-tile-label">up</span></button></div><div class="scales-pick-row"><button type="button" class="metro-play-grid-btn metro-play-grid-btn-reset"><span class="material-symbols-outlined" aria-hidden="true">skip_previous</span><span class="metroBlk-ctrl-value-label">previous</span></button><button type="button" class="metro-play-grid-btn metro-play-grid-btn-reset"><span class="material-symbols-outlined" aria-hidden="true">skip_next</span><span class="metroBlk-ctrl-value-label">next</span></button><button type="button" class="metro-play-grid-btn metro-play-grid-btn-reset"><span class="material-symbols-outlined" aria-hidden="true">shuffle</span><span class="metroBlk-ctrl-value-label">shuffle</span></button><button type="button" class="metro-play-grid-btn scales-pick-mine"><span class="material-symbols-outlined" aria-hidden="true">playlist_add_check</span><span class="metroBlk-ctrl-value-label">my scales</span></button></div><section class="flow-card scales-staff-card"><div class="scales-staff-head"><h2 class="scales-staff-title">D major scale</h2><span class="scales-staff-sub">1 octave · treble</span></div><div class="scales-staff">${NT.staff({ clef: 'treble', keySignature: { type: 'sharp', count: 2 }, stepRange: [-4, 11], noteGap: 2.4, justify: 60, label: 'D major scale', items: [{ type: 'timeSig', top: 4, bottom: 4 }, ...['D4', 'E4', 'F#4', 'G4', 'A4', 'B4', 'C#5', 'D5'].flatMap((p, i) => { const st = NT.staffStep(p, 'treble'); const n = { type: 'note', pitch: p, accidental: false, head: st >= 4 ? 'noteQuarterDown' : 'noteQuarterUp', cls: 'scales-note scales-note-' + i + (i === 2 ? ' is-now' : '') }; return i === 3 ? [n, { type: 'barline', glyph: 'barlineSingle' }] : [n]; }), { type: 'barline', glyph: 'barlineFinal' }] })}</div></section></div>`, measure: '.flow-tile, .scales-staff-title, .scales-staff-sub, .scales-note.is-now' },
                    { label: 'Key-note picker (.note-keyboard): the keyboard layout, D selected; keys a mode doesn\'t have are hidden (no D♯, G♯, A♯ major)', html: `<div class="note-keyboard">${['C#', 'F#', 'C', 'D', 'E', 'F', 'G', 'A', 'B', 'Db', 'Eb', 'Gb', 'Ab', 'Bb'].map(n => `<button type="button" class="flow-picker-tile${n === 'D' ? ' selected' : ''}" data-id="${n}"><span class="flow-picker-tile-icon-row">${n.replace('#', '♯').replace(/^([A-G])b$/, '$1♭')}</span></button>`).join('')}</div>`, measure: '.note-keyboard, .flow-picker-tile.selected' },
                ] : [] },
                { spec: 'timer-ring', title: 'Timer countdown ring', examples: [
                    { label: 'A 20-minute session, about a third gone, running (the tip pulses)', html: '<div class="timer-ring is-running" style="--ring-left:0.65"><svg class="timer-ring-svg" viewBox="0 0 120 120" aria-hidden="true"><circle class="timer-ring-track" cx="60" cy="60" r="50"/><circle class="timer-ring-arc" cx="60" cy="60" r="50" pathLength="100"/><circle class="timer-ring-lap-track" cx="60" cy="60" r="57"/><circle class="timer-ring-lap" cx="60" cy="60" r="57" pathLength="100"/><g class="timer-ring-tip"><circle cx="60" cy="10" r="5"/></g></svg><div class="timer-ring-readout"><div class="timer-ring-time">13:02</div><div class="timer-ring-label">Time left in session</div></div></div>', measure: '.timer-ring-arc, .timer-ring-time, .timer-ring-hour' },
                    { label: 'A 90-minute session, 15 minutes in: the outer ring holds the part over the hour and unwinds first', html: '<div class="timer-ring has-lap is-running is-long" style="--ring-left:1; --lap-left:0.25"><svg class="timer-ring-svg" viewBox="0 0 120 120" aria-hidden="true"><circle class="timer-ring-track" cx="60" cy="60" r="50"/><circle class="timer-ring-arc" cx="60" cy="60" r="50" pathLength="100"/><circle class="timer-ring-lap-track" cx="60" cy="60" r="57"/><circle class="timer-ring-lap" cx="60" cy="60" r="57" pathLength="100"/><g class="timer-ring-tip"><circle cx="60" cy="10" r="5"/></g></svg><div class="timer-ring-readout"><div class="timer-ring-time">1:15:00</div><div class="timer-ring-label">Time left in session</div><div class="timer-ring-hour">Hour 2 of 2</div></div></div>', measure: '.timer-ring-arc, .timer-ring-time, .timer-ring-hour' },
                ] },
                { spec: 'tuner', title: 'Tuner', examples: [
                    { label: 'Card states: idle / out of tune (gold) / in tune (green wash)', html: ['', 'out-of-tune', 'in-tune'].map((s, i) => `<div class="tuner-card ${s}"><div class="tuner-note-row"><span class="tuner-note">${['–', 'A', 'A'][i]}</span><span class="tuner-note-octave">${['', '4', '4'][i]}</span></div><div class="tuner-bar-wrap"><div class="tuner-bar-track"><div class="tuner-bar-zone"></div><div class="tuner-bar-center-mark"></div><div class="tuner-bar-needle ${s === 'in-tune' ? 'in-tune' : ''}" style="--needle-pos:${[50, 22, 52][i]}%"></div></div><div class="tuner-bar-scale"><span>-50</span><span>0</span><span>+50</span></div></div><div class="tuner-status">${['Play a note', 'Too flat', 'In tune'][i]}</div></div>`).join(''), measure: '.tuner-card.in-tune, .tuner-note, .tuner-bar-track, .tuner-bar-needle.in-tune' },
                ] },
                { spec: 'notation', title: 'Notation (Bravura)', examples: notationExamples },
                { spec: 'theory-quiz', title: 'Theory quiz', examples: [
                    { label: 'Quiz list rows (.theory-quiz-row on a clickable list row): Bravura icon, title, subtitle, then the last grade - or a "New" pill (.flow-pill-accent) for one not tried yet (ML-301)', html: `<div><p class="theory-intro">Short quizzes to learn music theory.</p><button type="button" class="history-item clickable theory-quiz-row"><span class="theory-quiz-icon">${NT ? nScale(NT.symbol('noteheadWhole'), 0.8) : ''}</span><span class="history-details"><strong>Note names</strong>Identify the note on a stave<br>Last grade 4 · yesterday</span>${theoryGrade(4)}</button><button type="button" class="history-item clickable theory-quiz-row"><span class="theory-quiz-icon">${NT ? nScale(NT.symbol('segno'), 0.8) : ''}</span><span class="history-details"><strong>Notation</strong>Symbols and speeds</span><span class="flow-pill flow-pill-accent">New</span></button></div>`, measure: '.theory-quiz-row, .theory-quiz-icon, .theory-grade-dot-on' },
                    { label: 'Note names with sharps and flats on (ML-292): the keyboard - sharps above the naturals, flats below, each in its black key\'s column, no E♯/B♯/C♭/F♭', html: `<div class="theory-answers theory-answers-keyboard">${["C#","D#","F#","G#","A#","C","D","E","F","G","A","B","Db","Eb","Gb","Ab","Bb"].map(n => theoryAnswer(n.replace('#', '♯').replace(/^([A-G])b$/, '$1♭'), null, n)).join('')}</div>`, measure: '.theory-answers-keyboard, .theory-answer' },
                    { label: 'Question: clock and tally, countdown, staff, a wrong answer (tapped: red + cross; right one: green + tick; "Not quite" line), 7 note buttons in one row (ML-292)', html: `<div><div class="theory-status"><span>0:42</span><span>14 right · 1 wrong</span></div><div class="theory-countdown"><div class="theory-countdown-fill" style="--remaining:0.7"></div></div><p class="theory-question">Which note is this?</p><div class="theory-prompt">${NT ? nScale(NT.staff({ clef: 'treble', items: [{ type: 'note', pitch: 'A5' }], stepRange: [-5, 13] }), 1.6) : ''}</div><p class="theory-feedback">Not quite: it's A</p><div class="theory-answers theory-answers-notes">${['C', 'D', 'E', 'F'].map(n => theoryAnswer(n)).join('')}${theoryAnswer('G', 'wrong')}${theoryAnswer('A', 'right')}${theoryAnswer('B')}</div></div>`, measure: '.theory-status, .theory-countdown-fill, .theory-question, .theory-feedback, .theory-answer, .theory-answer-right, .theory-answer-wrong' },
                ...
                { spec: 'flow-editor', title: 'Flow editor', examples: [
                    ...
                    { label: 'Warning tile (.flow-tile-warning)', html: '<div class="flow-tile-grid flow-tile-grid-3"><div class="flow-tile">Bar 1</div><div class="flow-tile flow-tile-warning">Bar 2</div><div class="flow-tile">Bar 3</div></div>', measure: '.flow-tile-warning' },
                ] },
                ...
                { spec: 'admin-shell', title: 'Admin panel', examples: [
                    { label: 'Phone-width menu head row (.admin-sidebar-head, .admin-sidebar-current, .admin-nav-toggle) - 700px and below; the ☰ opens the section list (.admin-nav-items)', wide: true, html: '<div class="admin-design-static admin-design-phone"><div class="admin-sidebar-head"><div class="admin-sidebar-title">Admin<span class="admin-sidebar-current">Features</span></div><button type="button" class="admin-nav-toggle" aria-expanded="false" aria-label="Show admin sections"><span class="material-symbols-outlined" aria-hidden="true">menu</span></button></div></div>', measure: '.admin-nav-toggle, .admin-sidebar-current' },
    ...
    // Resolves a token's value to what the browser computes (rgb for a colour, the shadow list) so it
    // can be matched against an element's computed style. The value goes in as a custom property that
    // .admin-design-probe reads (ML-288).
    const probe = document.createElement('span');
    probe.className = 'admin-design-probe';
    ...
    function asColor(v) {
        ...
        probe.style.setProperty('--probe-color', v);
    ...
    function buildLookup() {
        ...
        for (const name of all) {
            ...
            else if (/^--(shadow|focus)/.test(name)) { probe.style.setProperty('--probe-shadow', v); put(lookup.shadow, getComputedStyle(probe).boxShadow, name); probe.style.removeProperty('--probe-shadow'); }
    ...
    function foundationsHtml() {
        ...
        const swatch = (r) => `<div class="admin-design-swatch"><div class="admin-design-swatch-color" style="--sample: var(${r.name})"></div><div class="admin-design-swatch-label"><code>${r.name}</code><span class="admin-design-token-value" data-token="${r.name}"></span><small>${r.comment}</small></div></div>`;
        ...
        return `
            ...
            ${table(rows(/^--font-(2xs|xs|sm|base|md|lg|xl|2xl|3xl)$/), r => `<span class="admin-design-sample-size" style="--sample: var(${r.name})">The quick brown fox</span>`)}
            ${table(rows(/^--font-weight-/), r => `<span class="admin-design-sample-weight" style="--sample: var(${r.name})">The quick brown fox</span>`)}
            ${table(rows(/^--font-(sans|mono|music)$/), r => `<span class="admin-design-sample-family" style="--sample: var(${r.name})">Aa 𝄋 𝄌 0123</span>`)}
            ${table(rows(/^--line-height-/), r => `<span class="admin-design-leading" style="--sample: var(${r.name})">Two lines of text<br>to show the leading</span>`)}
            ${table(rows(/^--icon-/), r => `<span class="material-symbols-outlined admin-design-sample-size" style="--sample: var(${r.name})">music_note</span>`)}
            <h3 class="admin-design-subhead">Spacing</h3>
            ${table(rows(/^--space-/), r => `<span class="admin-design-space-bar" style="--sample: var(${r.name})"></span>`)}
            <h3 class="admin-design-subhead">Radius</h3>
            <div class="admin-design-swatches">${rows(/^--radius-/).map(r => `<div class="admin-design-shape" style="--sample: var(${r.name})"><code>${r.name}</code><span class="admin-design-token-value" data-token="${r.name}"></span></div>`).join('')}</div>
            <h3 class="admin-design-subhead">Elevation</h3>
            <div class="admin-design-swatches">${rows(/^--(shadow|focus)/).map(r => `<div class="admin-design-shadow" style="--sample: var(${r.name})"><code>${r.name}</code><small>${r.comment}</small></div>`).join('')}</div>
            ${table(rows(/^--z-/))}
            <h3 class="admin-design-subhead">Motion</h3>
            <p class="admin-intro">Hover (or tap) a row to play its duration.</p>
            ${table(rows(/^--duration-/), r => `<span class="admin-design-motion-track"><span class="admin-design-motion-dot" style="--sample: var(${r.name})"></span></span>`)}
            <h3 class="admin-design-subhead">Layout</h3>
            ${table(rows(/^--(app-max-width|touch-target|bottom-bar-)/))}`;
    }

    // ------------------------------------------------------------------ what's new (design sign-off)
    // npm run design-gate writes public/design-new.json: everything new to the design system since the
    // last sandbox release (classes, tokens, specs). When it's there, this page can show only the
    // examples that use something new, each marked NEW with the new classes it uses, so the owner signs
    // off on what they can see (scripts/design-signoff-shots.mjs screenshots this view for the chat).

    let NEW = null;
    async function loadWhatsNew() {
        try {
            const res = await fetch('design-new.json', { cache: 'no-store' });
    ...
    function whatsNewHtml() {
        ...
        const when = NEW.generatedAt ? new Date(NEW.generatedAt).toLocaleString() : '';
        const tokenRows = tokens.map(t => `<tr><td><span class="admin-design-new-swatch" style="--sample: var(${t.name})"></span></td><td><code>${t.name}</code></td><td>${esc(t.value)}</td><td>${t.was ? 'was ' + esc(t.was) : 'new'}</td></tr>`).join('');
        return `<div class="admin-design-new-panel" id="designWhatsNew">
            <h2 class="admin-design-new-title"><span class="admin-design-new-badge">NEW</span> What's new since the last sandbox release</h2>
            <p class="admin-intro">Compared with <code>${esc(NEW.base || '')}</code>${when ? ', listed ' + esc(when) : ''} by <code>npm run design-gate</code>.
                <span id="designNewCounts"></span> Switch on <strong>Only what's new</strong> to hide everything else.</p>
            ${tokens.length ? `<h3 class="admin-design-minihead">New or changed tokens (${tokens.length})</h3><div class="admin-stat-table-wrap"><table class="admin-stat-table admin-design-table"><tbody>${tokenRows}</tbody></table></div>` : ''}
            ${(NEW.tokensRemoved || []).length ? `<h3 class="admin-design-minihead">Removed tokens</h3><p>${NEW.tokensRemoved.map(t => `<code>${t}</code>`).join(' ')}</p>` : ''}
            <div id="designNewUnshown"></div>
        </div>`;
    }

    // Marks every example that uses a new class (and every example of a brand-new spec), its section and
    // its group; lists the new classes no example shows yet, so nothing new goes unseen.
    function markNew(container) {
        const newSet = new Set(NEW.classes || []);
        const shown = new Set();
        const specsNew = new Set(NEW.specsNew || []);
        const specsChanged = new Set(NEW.specsChanged || []);
        container.querySelectorAll('.admin-design-section').forEach(section => {
            const spec = section.dataset.spec;
            section.querySelectorAll('.admin-design-example').forEach(ex => {
                const used = new Set();
                ex.querySelectorAll('.admin-design-stage *').forEach(el => el.classList.forEach(c => { if (newSet.has(c)) used.add(c); }));
                if (!used.size && !specsNew.has(spec)) return;
                used.forEach(c => shown.add(c));
                ex.classList.add('admin-design-is-new');
                const label = ex.querySelector('.admin-design-example-label');
                label.insertAdjacentHTML('afterbegin', '<span class="admin-design-new-badge">NEW</span>');
                if (used.size) label.insertAdjacentHTML('afterend', `<div class="admin-design-new-classes">${[...used].sort().map(c => `<code>.${c}</code>`).join('')}</div>`);
            });
            const isNew = specsNew.has(spec) || section.querySelector('.admin-design-example.admin-design-is-new');
            if (isNew) section.classList.add('admin-design-is-new');
            if (specsNew.has(spec) || specsChanged.has(spec)) section.querySelector('.admin-design-section-head h3').insertAdjacentHTML('beforeend', ` <span class="admin-design-new-badge">${specsNew.has(spec) ? 'NEW spec' : 'spec changed'}</span>`);
        });
        container.querySelectorAll('.admin-design-group-wrap').forEach(g => { if (g.querySelector('.admin-design-section.admin-design-is-new')) g.classList.add('admin-design-is-new'); });
        const unshown = [...newSet].filter(c => !shown.has(c)).sort();
        container.querySelector('#designNewCounts').textContent = `${newSet.size} new class${newSet.size === 1 ? '' : 'es'}: ${shown.size} shown in the examples below (marked NEW)${unshown.length ? `, ${unshown.length} not in any example yet` : ''}.`;
        if (!unshown.length) return;
        const bySpec = {};
        unshown.forEach(c => { const spec = (NEW.classSpecs || {})[c] || 'other'; (bySpec[spec] = bySpec[spec] || []).push(c); });
        container.querySelector('#designNewUnshown').innerHTML = `<h3 class="admin-design-minihead">Not in any example yet (${unshown.length}) - ask for a picture of these</h3>` +
            Object.keys(bySpec).sort().map(spec => `<div class="admin-design-new-classes"><b>${spec}</b>${bySpec[spec].map(c => `<code>.${c}</code>`).join('')}</div>`).join('');
    }

    // ------------------------------------------------------------------ render

    const esc = (s) => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;'); // &quot; too: a measure selector can hold [attr="value"]
    ...
    function render(container) {
        ...
        container.innerHTML = `
            <div class="admin-design-toolbar">
                ...
                ${NEW ? `<label class="admin-design-toggle"><span>Only what's new</span><span class="toggle-switch"><input type="checkbox" id="designOnlyNew"><span class="toggle-slider"></span></span></label>` : ''}
            </div>
            ${NEW ? whatsNewHtml() : ''}
            <div class="admin-design-jumps"><a class="admin-link admin-design-jump" href="#design-group-Foundations">Foundations</a>${nav}</div>
            <div class="admin-design-foundations">
            ...
            ${GROUPS.map(g => `<div class="admin-design-group-wrap"><h2 class="admin-design-group" id="design-group-${g.title.replace(/\W+/g, '-')}">${g.title}</h2>${g.note ? `<p class="admin-intro">${g.note}</p>` : ''}${g.items.map(sectionHtml).join('')}</div>`).join('')}`;
        if (NEW) markNew(container);
        ...
        container.querySelector('#designOnlyNew')?.addEventListener('change', (e) => container.classList.toggle('admin-design-only-new', e.target.checked));
    ...
    async function open() {
        ...
        rendered = true;
        try {
            [TOKENS, NEW] = await Promise.all([loadTokens(), loadWhatsNew()]);
            render(container);
        } catch (err) {
            rendered = false;
