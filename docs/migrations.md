...
| `052_theory_quiz.sql` | Adds `theory_quiz_attempts` + `theory_quiz_answers` (ML-265) - finished Theory practice rounds (ML-260) and every answer in them. Scores/grades are recomputed server-side with `public/theoryEngine.js`, never taken from the client; `settings_key` groups comparable rounds for history and bests. `session_segment_id` is there, nullable, for the later link to practice sessions. Seeds the `theory_practice` feature row (enabled). See `docs/theory-practice.md`. |
| `053_theory_smart_learn.sql` | Adds `theory_question_weights` (ML-269) - Theory Smart learn: a per-person, per-question weight 0-10 (wrong +2, right −1) plus lifetime right/wrong counts, updated by the server from every finished round while the `theory_smart_learn` feature is on. Seeds that feature row (enabled; intended for the paid tier). See `docs/theory-practice.md` ("Smart learn"). |
| `054_scales_practice.sql` | Seeds the `scales_practice` feature row (ML-9, enabled) behind the home screen Scales tool. No tables: a scale's settings and "My scales" are stored on the device (`tml.scales`). |
